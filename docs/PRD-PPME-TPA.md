...
*   **Design Validation (Figma Make prototype, reviewed 2026-07-01):**
    ...
    *   **Open for follow-up:** notification center/list screen not yet reviewed — **still open, and the screen now exists**: it was built in Milestone 7 part 3 (TAD ADR-017) against a schema deliberately designed to survive whatever the review decides, so regrouping, reordering, filtering or rewording it needs no migration. Reviewing it is still outstanding; language toggle (ID/NL) destination behavior not yet verified; "date badges overlaid on cards" from the design direction above appear as plain text timestamps in the current prototype rather than overlaid badges (minor, cosmetic).
