...
- **Who inside the organization can see what:** access is need-to-know by role and
  ...
  assigned to; a 16+ student sees only their own record, and cannot change it. (A 16+ student who *also* assists with a class — a student assistant — may record for that class like any other tutor of it, and still not for themselves: TAD ADR-020. Read-only follows from the relationships a person holds, not from the label on their account.) **The `admin`
