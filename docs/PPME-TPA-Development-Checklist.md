...
- [~] Top nav: logo left, language toggle (globe icon), notification bell with badge — logo + language toggle done; **notification bell still not built**. It belongs with the in-app notification centre it would open (TAD ADR-015 part 3), not with the push pipeline: a bell with a badge implies a stored, readable list of notifications, and that needs a new table. Notification *settings* are reachable today from the dashboard instead
...
- [ ] Notification center/list screen — design still not reviewed in the prototype batch, so still not built (TAD ADR-015 part 3). Confirmed before building, as this line asked: it needs a new table, RLS policies and pgTAP cases, and locking a schema around an unreviewed screen is the specific risk worth avoiding. The in-app copy it will use is already drafted (`notifications.*`)
