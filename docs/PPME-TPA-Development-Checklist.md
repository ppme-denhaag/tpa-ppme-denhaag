...
**Status as of this writing:** 1–3 done. 4 done (Hadir). 5 done (Tugas + Yanbu'a + Al-Quran + Murajaah, all built). 6 done for all five built flows (Hadir + Tugas + Yanbu'a + Al-Quran + Murajaah). 7 works structurally (RLS + the family views already handle `role=student`) but hasn't been exercised with a real 16+ self-login account. 8 barely started — `invite-user.mts` exists but none of the 5 originally-planned notification/streak Functions do; FR-005 (homework due-date reminders), the Quran milestone-celebration notification, and Murajaah's FR-006 (daily practice reminders) are all deliberately deferred with the rest of notifications, same reasoning as Milestone 1. 9 not started. 10 not started. Admin enrollment (§10) and Homework Assignments (§11) both landed out of the numbered order, in response to practical need rather than as scheduled milestones; Quran (§12) and Murajaah (§13 above) both landed in their intended §5 slot.
