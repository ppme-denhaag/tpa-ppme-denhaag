...
| Notification Spec | Web Push (VAPID) for absence alerts, homework reminders, milestone celebrations; optional WhatsApp via Business API (Phase 3). Every notification in the Spec below is built, plus a weekly digest the Scheduler table asked for, plus the in-app notification centre every one of them also writes to (ADR-017). Milestone 7's notification work is complete |
