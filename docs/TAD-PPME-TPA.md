...
* **Service Worker**: Workbox precaching for app shell; runtime caching for API responses. Offline submissions (attendance, murajaah) are queued and replayed from the **app**, not the service worker — ADR-029, which supersedes ADR-005's original service-worker-level Background Sync plan (Chrome/Android-only, and no way to refresh an expired session before replay)
